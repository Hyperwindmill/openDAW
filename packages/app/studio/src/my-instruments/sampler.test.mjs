...
const src = readFileSync(new URL("./sampler.js", import.meta.url), "utf8")
...
// 1) Sustained note: finite, bounded, audible (loop keeps it going)
{
...
// 2) Slow attack: amplitude rises gradually
{
    ...
    const r = record(p, 1.0, [{at: 0, fn: pr => pr.noteOn(60, 0.9, 0, 1)}])
    const energy = (startSec) => { let e = 0; const n = Math.floor(0.1 * sampleRate); const start = Math.floor(startSec * sampleRate); for (let i = start; i < start + n; i++) e += r.out[i] * r.out[i]; return e }
    const early = energy(0), late = energy(0.4)
    check("slow attack rises", late > early * 2, `early=${early.toExponential(2)} late=${late.toExponential(2)}`)
}
// 3) noteOff releases and culls to silence
{
    const p = new Processor()
...
// 4) Poly + reset
{
    ...
    const r = record(p, 0.6, [
        {at: 0, fn: pr => pr.noteOn(57, 0.8, 0, 1)},
        {at: 0, fn: pr => pr.noteOn(60, 0.8, 0, 2)},
        {at: 0, fn: pr => pr.noteOn(64, 0.8, 0, 3)},
...
// 5) Voice-steal cap at 8
{
    const p = new Processor()
    const events = []
    for (let n = 0; n < 12; n++) events.push({at: 0, fn: pr => pr.noteOn(48 + n, 0.7, 0, n + 1)})
    const r = record(p, 0.2, events)
    check("voice-steal cap <= 8", r.maxVoices <= 8, `maxVoices=${r.maxVoices}`)
    check("steal finite/bounded", !r.anyNaN && r.peak <= 1.0, `peak=${r.peak.toFixed(3)}`)
}
